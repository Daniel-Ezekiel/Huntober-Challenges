...
const mashUpFix = str => {
    ...
    const noEek = noSmallC.split('Eek!').join('');
    const reversed = noEek.split('').reverse().join('');
    const vSplit = reversed.split('v');

    const res = vSplit.slice(1).map(el => el[0]);
